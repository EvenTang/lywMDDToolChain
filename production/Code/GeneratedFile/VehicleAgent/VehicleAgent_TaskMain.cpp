...
uint32_t VehicleAgent_Task_Main(void * params) {
    while(1) {
        // Recieve Msg Buffer
		...
		if (buff_size >= 0) {
			...
			VehicleAgentEvent msg;
			msg.ParseFromString(p_msg_buff);
			VehicleAgent_Event msg_id = msg.GetMessageID();
			VehicleAgent_State stm_state = GetSTMState();
			// todo : add range check
			// dispatch msg to ECBs
			VehicleAgent_state_ecb_map[stm_state][msg_id](msg);
			delete [] p_msg_buff;			
		}
    }
}
